...
public class RenderJSBlockRender {
    public static void renderSingleBlock(BlockState pState, PoseStack pPoseStack, MultiBufferSource pBufferSource, int r, int g, int b, int a, int pPackedLight, int pPackedOverlay, ModelData modelData, net.minecraft.client.renderer.RenderType renderType) {
            ...
            switch (rendershape) {
                case MODEL:
                    ...
                    for (net.minecraft.client.renderer.RenderType rt : bakedmodel.getRenderTypes(pState, RandomSource.create(42), modelData))
                        renderModel(pPoseStack.last(), pBufferSource.getBuffer(renderType != null ? renderType : RenderTypeHelper.getEntityRenderType(rt, false)), pState, bakedmodel, (float) r / 255, (float) g / 255, (float) b / 255, (float) a / 255, pPackedLight, pPackedOverlay, modelData, rt);
                    break;
    ...
    public static void renderModel(PoseStack.Pose pPose, VertexConsumer pConsumer, @Nullable BlockState pState, BakedModel pModel, float r, float g, float b, float a, int pPackedLight, int pPackedOverlay, ModelData modelData, net.minecraft.client.renderer.RenderType renderType) {
